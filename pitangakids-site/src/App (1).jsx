...
const LANG = {
  pt: {
    ...
    planos: [
      { nome: "Semente", emoji: "🌱", preco: "€9.99/mês", cor: C.mint, corT: "#2A7A6E",
  ...
  en: {
    ...
    privSecoes: [
      { t:"Information Collection", p:"Pitanga Kids may collect basic account information such as email address when users choose to sign in using email or Google (Gmail). This information is used solely for authentication and access to the app." },
      ...
      { t:"Account Deletion", p:"Users can request account deletion directly within the app or by contacting: pitangakids.app@outlook.com." },
  ...
  es: {
    ...
    colecoes: [
      { nome: "Colección Tulipa", emoji: "🌷", cor: C.rosa, idade: "2–5 años", status: "Disponible",
        desc: "Aventuras de Nico y Stella — historias de amor, familia y descubrimientos.",
    ...
    artigos: [
      { id:1, emoji:"📖", cor:C.mint, tempo:"4 min", data:"Mayo 2026", tags:["Lectura","Desarrollo"],
        ...
        resumo:"La lectura compartida es una de las experiencias más poderosas que puedes ofrecer a tu hijo." },
      { id:2, emoji:"🎯", cor:C.rosa, tempo:"5 min", data:"Abril 2026", tags:["Montessori","Casa"],
        titulo:"Qué es Montessori en la práctica y cómo aplicarlo en casa",
        resumo:"Creada por Maria Montessori, esta metodología cree que los niños aprenden mejor cuando tienen libertad para explorar a su propio ritmo." },
      { id:3, emoji:"🤸", cor:C.lilas, tempo:"5 min", data:"Marzo 2026", tags:["Movimiento","Actividades"],
        ...
        resumo:"El movimiento es el primer idioma del niño. ¡Desarrollar la coordinación es esencial para la cognición y la autoconfianza!" },
      { id:4, emoji:"❤️", cor:C.coral, tempo:"4 min", data:"Febrero 2026", tags:["Emociones","Neurociencia"],
        ...
        resumo:"Los niños que se sienten seguros y amados aprenden mejor, más rápido y con más alegría." },
      { id:5, emoji:"🌳", cor:C.mint, tempo:"5 min", data:"Enero 2026", tags:["Movimiento","Cognición"],
        ...
        resumo:"¡El movimiento no es lo opuesto al aprendizaje — es parte esencial de él!" },
    ],
    sobreH: ["Nuestra ", "historia"],
    sobreQuote: "\"Pitanga Kids nació de una creencia simple: que cada niño lleva dentro de sí una luz única, esperando ser vista, celebrada y nutrida con afecto.\"",
    sobreCards: [
    ...
    privSecoes: [
      { t:"Recopilación de información", p:"Pitanga Kids puede recopilar información básica de cuenta, como dirección de correo electrónico, cuando los usuarios eligen iniciar sesión mediante correo electrónico o Google." },
      { t:"Privacidad de los niños", p:"La creación de cuenta debe ser realizada por un padre, madre o tutor legal. No recopilamos intencionalmente datos personales sensibles de niños." },
      { t:"Uso de la información", p:"La información recopilada se utiliza únicamente para permitir el acceso a la aplicación y personalizar la experiencia." },
      ...
      { t:"Inteligencia Artificial", p:"Ningún dato personal se utiliza para entrenar modelos de IA." },
      { t:"Eliminación de cuenta", p:"Contacto: pitangakids.app@outlook.com" },
...
const globalStyles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero-img {
    ...
    border-radius: 24px;
    object-fit: cover;
    box-shadow: 0 6px 28px rgba(196,168,224,0.22);
    margin: 0 auto;
    display: block;
  }
  @media (min-width: 640px) {
  ...
  .nav-links {
    ...
    border-bottom: 2px solid #F7A8C0;
...
function Img({ src, alt, style, className }) {
  ...
  if (erro) {
    return (
      <div style={{
        ...style,
        background: `linear-gradient(135deg, ${C.creme2}, ${C.rosa}22)`,
        display: "flex", alignItems: "center", justifyContent: "center",
        flexDirection: "column", gap: 6,
      }} className={className}>
        <span style={{ fontSize: 32 }}>🌸</span>
        <span style={{ fontSize: 11, color: C.suave }}>{alt}</span>
      </div>
    );
  }
  return (
...
function Nav({ lang, setLang, page, setPage, t }) {
  ...
  const fechar = (id) => { setPage(id); setAberto(false); };
    ...
    <>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", flexShrink: 0 }}
          onClick={() => fechar("home")}>
          <Img src={IMGS.logo} alt="Pitanga Kids" style={{ height: 36, borderRadius: 10 }} />
          <span style={{ fontFamily: "'Baloo 2',cursive", fontWeight: 900, fontSize: 17, color: C.coral }}>
            Pitanga Kids
          </span>
        </div>
        ...
        <button
          ...
          style={{
            ...
            fontSize: 22, padding: 4, color: C.texto,
        ...
        <div className={`nav-links${aberto ? " open" : ""}`}>
          {navIds.map((id, i) => (
            <button key={id} onClick={() => fechar(id)} style={{
          ...
          <div className="nav-langs">
            {["pt","en","es"].map(l => (
              <button key={l} onClick={() => { setLang(l); setAberto(false); }} style={{
...
function Footer({ t, setPage }) {
  ...
  return (
    <footer style={{ background: C.texto, color: "#fff", padding: "28px 20px", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <a href="https://instagram.com/pitangakids.app" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, textDecoration: "none" }}>📸 @pitangakids.app</a>
        <a href="https://tiktok.com/@pitangakids.app" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, textDecoration: "none" }}>🎵 @pitangakids.app</a>
      </div>
    ...
    <div>
      ...
      <div className="hero-grid">
        <div>
          ...
          <h1 style={{ fontFamily: "'Baloo 2',cursive", fontSize: "clamp(24px,5vw,48px)", fontWeight: 900, color: C.texto, lineHeight: 1.15, marginBottom: 14 }}>
            {t.heroH1[0]}<span style={{ color: C.coral }}>{t.heroH1[1]}</span>
            {t.heroH1[2]}<span style={{ color: C.lilas }}>{t.heroH1[3]}</span>
          </h1>
          <p style={{ fontSize: 15, color: C.suave, maxWidth: 460, lineHeight: 1.7 }}>{t.heroDesc}</p>
        </div>
        <Img src={IMGS.abraco} alt="Stella e Nico" className="hero-img" style={{
      ...
      {/* Personagens */}
      <div style={{ background: C.creme2 }} className="section-pad">
        <h2 style={{ textAlign: "center", fontFamily: "'Baloo 2',cursive", fontSize: 24, fontWeight: 800, color: C.texto, marginBottom: 28 }}>
        ...
        <div className="cards-grid cards-grid-3" style={{ maxWidth: 860, margin: "0 auto" }}>
          {[
      ...
      {/* Metodologias */}
      <div style={{ background: C.creme }} className="section-pad">
        <h2 style={{ textAlign: "center", fontFamily: "'Baloo 2',cursive", fontSize: 24, fontWeight: 800, color: C.texto, marginBottom: 28 }}>
          {t.metH[0]}<span style={{ color: C.lilas }}>{t.metH[1]}</span>
        </h2>
        <div className="cards-grid cards-grid-4" style={{ maxWidth: 860, margin: "0 auto" }}>
          {t.met.map(m => (
...
function AppPage({ t }) {
  return (
    <div>
      <div className="section-header" style={{ background: "linear-gradient(135deg,#FFF0F6,#F5EFFC)" }}>
        <h1 style={{ fontFamily: "'Baloo 2',cursive", fontSize: "clamp(26px,5vw,36px)", fontWeight: 900, color: C.texto, marginBottom: 10 }}>
          {t.appH[0]}<span style={{ color: C.coral }}>{t.appH[1]}</span>
        </h1>
        <p style={{ color: C.suave, fontSize: 15, maxWidth: 500, margin: "0 auto 24px" }}>{t.appDesc}</p>
      ...
      <div style={{ background: C.creme }} className="section-pad">
        <h2 style={{ textAlign: "center", fontFamily: "'Baloo 2',cursive", fontSize: 22, fontWeight: 800, color: C.texto, marginBottom: 28 }}>
          {t.appPlanosH}
        </h2>
        <div className="planos-grid">
          {t.planos.map(p => (
            <div key={p.nome} style={{
              ...
              <div style={{ fontFamily: "'Baloo 2',cursive", fontSize: 24, fontWeight: 900, color: p.corT, marginBottom: 16 }}>{p.preco}</div>
  ...
  return (
    <div>
      <div className="section-header" style={{ background: "linear-gradient(135deg,#FFF0F6,#F5EFFC)" }}>
        ...
        <p style={{ color: C.suave, fontSize: 15, maxWidth: 500, margin: "0 auto" }}>{t.livrosDesc}</p>
      </div>
      <div style={{ background: C.creme }} className="section-pad">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 780, margin: "0 auto" }}>
          {t.colecoes.map(c => {
            const disponivel = ["Disponível","Available","Disponible"].includes(c.status);
            return (
              <div key={c.nome} style={{
                background: C.branco, borderRadius: 22, padding: "22px 20px",
                boxShadow: "0 4px 14px rgba(0,0,0,0.05)",
                border: `2px solid ${c.cor}`,
              }}>
                <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                  <div style={{ width: 52, height: 52, borderRadius: 14, background: c.cor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>{c.emoji}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", gap: 7, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
                      <h3 style={{ fontFamily: "'Baloo 2',cursive", fontSize: 18, fontWeight: 800, color: C.texto, margin: 0 }}>{c.nome}</h3>
                      <span style={{ background: disponivel ? C.mint : C.creme, color: disponivel ? "#2A7A6E" : C.suave, fontSize: 11, fontWeight: 700, padding: "2px 9px", borderRadius: 50 }}>{c.status}</span>
                      <span style={{ fontSize: 11, color: C.suave, background: C.creme, padding: "2px 8px", borderRadius: 50 }}>👶 {c.idade}</span>
                    </div>
                    <p style={{ color: C.suave, fontSize: 13.5, lineHeight: 1.6, marginBottom: 10 }}>{c.desc}</p>
                    ...
                    {disponivel && (
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                        <a href={LINKS.amazon} target="_blank" rel="noopener noreferrer" style={{
                          ...
                          fontFamily: "'Baloo 2',cursive", fontWeight: 700, fontSize: 13, textDecoration: "none",
                        }}>{t.btnAmazon}</a>
                        <a href={LINKS.bookvault} target="_blank" rel="noopener noreferrer" style={{
                          ...
                          border: `1.5px solid ${C.lilas}`, borderRadius: 50, padding: "8px 18px",
                          fontFamily: "'Baloo 2',cursive", fontWeight: 700, fontSize: 13, textDecoration: "none",
                        }}>{t.btnBookvault}</a>
                      </div>
                    )}
...
function BlogPage({ t }) {
  return (
    <div>
      <div className="section-header" style={{ background: "linear-gradient(135deg,#EAF8F4,#F5EFFC)" }}>
        <h1 style={{ fontFamily: "'Baloo 2',cursive", fontSize: "clamp(26px,5vw,36px)", fontWeight: 900, color: C.texto, marginBottom: 10 }}>
          {t.blogH[0]}<span style={{ color: C.coral }}>{t.blogH[1]}</span>
        </h1>
        <p style={{ color: C.suave, fontSize: 15, maxWidth: 500, margin: "0 auto" }}>{t.blogDesc}</p>
      </div>
      <div style={{ background: C.creme }} className="section-pad">
        <div className="blog-grid">
          {t.artigos.map(a => (
            <div key={a.id} style={{
              ...
              <div style={{ background: `${a.cor}28`, padding: "20px 16px 14px", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ padding: "14px 16px 18px" }}>
                <h3 style={{ fontFamily: "'Baloo 2',cursive", fontSize: 15, fontWeight: 800, color: C.texto, marginBottom: 7, lineHeight: 1.35 }}>{a.titulo}</h3>
                <p style={{ color: C.suave, fontSize: 13, lineHeight: 1.6, marginBottom: 10 }}>{a.resumo}</p>
    ...
    <div>
      ...
      <div style={{ background: C.creme }} className="section-pad">
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <div style={{
            background: C.branco, borderRadius: 22, padding: "28px 24px",
            boxShadow: "0 3px 14px rgba(0,0,0,0.04)",
            borderLeft: `5px solid ${C.lilas}`, marginBottom: 24,
          }}>
            <p style={{ fontSize: 15.5, color: C.texto, lineHeight: 1.8, textAlign: "center", fontStyle: "italic" }}>{t.sobreQuote}</p>
          </div>
          <div className="sobre-cards">
            {t.sobreCards.map(c => (
              <div key={c.t} style={{
                background: C.branco, borderRadius: 16, padding: "18px",
                boxShadow: "0 3px 10px rgba(0,0,0,0.04)", textAlign: "center",
              }}>
                <div style={{ fontSize: 26, marginBottom: 7 }}>{c.icon}</div>
                <h3 style={{ fontFamily: "'Baloo 2',cursive", fontWeight: 800, fontSize: 14, color: C.texto, marginBottom: 5 }}>{c.t}</h3>
...
function PitaguinhasPage({ t }) {
  return (
    <div>
      <div className="section-header" style={{ background: "linear-gradient(135deg,#FFF0F6,#FFF0E8)" }}>
        <div style={{ fontSize: 48, marginBottom: 10 }}>🍊</div>
        <h1 style={{ fontFamily: "'Baloo 2',cursive", fontSize: "clamp(26px,5vw,36px)", fontWeight: 900, color: C.texto, marginBottom: 10 }}>
          {t.pitH[0]}<span style={{ color: C.coral }}>{t.pitH[1]}</span>{t.pitH[2]}
        </h1>
        <p style={{ color: C.suave, fontSize: 15, maxWidth: 500, margin: "0 auto" }}>{t.pitDesc}</p>
      </div>
      <div style={{ background: C.creme }} className="section-pad">
        <div className="cards-grid cards-grid-4" style={{ maxWidth: 820, margin: "0 auto" }}>
          {t.pitCards.map(v => (
            <div key={v.t} style={{
              background: C.branco, borderRadius: 20, padding: "22px 16px",
              boxShadow: "0 3px 12px rgba(0,0,0,0.05)", textAlign: "center",
              border: `2px solid ${C.coral}22`,
            }}>
              <div style={{ fontSize: 34, marginBottom: 8 }}>{v.icon}</div>
              <h3 style={{ fontFamily: "'Baloo 2',cursive", fontWeight: 800, fontSize: 15, color: C.texto, marginBottom: 5 }}>{v.t}</h3>
              <p style={{ color: C.suave, fontSize: 13, lineHeight: 1.6 }}>{v.d}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      <div className="section-header" style={{ background: "linear-gradient(135deg,#EAF8F4,#F5EFFC)" }}>
        ...
        <p style={{ color: C.suave, fontSize: 15, maxWidth: 500, margin: "0 auto" }}>{t.recursosDesc}</p>
      </div>
      <div style={{ background: C.creme }} className="section-pad">
        <div className="cards-grid cards-grid-4" style={{ maxWidth: 920, margin: "0 auto" }}>
          {t.atividades.map(a => (
            <div key={a.nome} style={{
              ...
              <div style={{ background: `${a.cor}28`, padding: "20px 16px 16px", textAlign: "center" }}>
                <div style={{ fontSize: 38, marginBottom: 6 }}>{a.emoji}</div>
                <h3 style={{ fontFamily: "'Baloo 2',cursive", fontWeight: 800, fontSize: 16, color: C.texto, marginBottom: 6 }}>{a.nome}</h3>
              ...
              <div style={{ padding: "14px 16px 18px" }}>
                <p style={{ color: C.suave, fontSize: 13, lineHeight: 1.6, marginBottom: 12 }}>{a.desc}</p>
                <button style={{
...
function PrivacidadePage({ t }) {
  return (
    <div>
      <div className="section-header" style={{ background: C.creme2 }}>
        <h1 style={{ fontFamily: "'Baloo 2',cursive", fontSize: "clamp(24px,5vw,32px)", fontWeight: 900, color: C.texto, marginBottom: 7 }}>
          {t.privH[0]}<span style={{ color: C.lilas }}>{t.privH[1]}</span>
        </h1>
        <p style={{ color: C.suave, fontSize: 13 }}>{t.privSub}</p>
      </div>
      <div style={{ background: C.creme }} className="section-pad">
        <div style={{ maxWidth: 700, margin: "0 auto", background: C.branco, borderRadius: 20, padding: "28px 20px", boxShadow: "0 3px 12px rgba(0,0,0,0.04)" }}>
          <p style={{ fontStyle: "italic", color: C.suave, marginBottom: 22, fontSize: 14 }}>{t.privIntro}</p>
          {t.privSecoes.map((s, i) => (
            <div key={s.t} style={{ marginBottom: 18, paddingBottom: 18, borderBottom: i < t.privSecoes.length - 1 ? `1px solid ${C.rosa}44` : "none" }}>
              <h3 style={{ fontFamily: "'Baloo 2',cursive", fontWeight: 800, fontSize: 15, color: C.texto, marginBottom: 5 }}>{s.t}</h3>
